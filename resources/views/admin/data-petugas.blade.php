<x-app-layout>
    ...
    <x-slot name="slot">
        ...
        <div class="flex md:flex-row flex-col my-3 items-end items-center justify-around">
                            ...
                            <div
                                ...
                                <div x-cloak x-show="modelOpen"
                                    ...
                                    <div class="flex items-center justify-between space-x-4">
                                        <h1 class="text-xl font-medium text-gray-800 ">Tambah Data Petugas</h1>
                                    </div>

                                    <form class="mt-5" action="{{ route('data-petugas.store') }}" method="post"
                                        x-data="{ alamat: '' }">
                                        @csrf
                                        <div>
                                        ...
                                        <div class="mt-4">
                                            <input placeholder="Password Minimal 8 Kata" type="password" required
                                                name="password"
                                                class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                                            @error('password')
                                                <div class="text-red-500 mb-2 text-sm" x-init="modelOpen = true">
            ...
            <form action="{{ route('data-petugas-search') }}" class="flex items-center my-1" method="get">
                ...
                <input type="text" name="name" placeholder="Cari Nama.."
                    class="h-10 rounded-r-full w-24 sm:w-full p-2">
            </form>
        </div>
            ...
            <table class="border-collapse border border-slate-400 container mx-auto w-10/12">
                <thead>
                    <tr style="background-color: #116D6E" class="text-white">
                        ...
                        <th class="border border-neutral-900 p-2">Email</th>
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr class="text-center" x-data="{ detailOpen: false, hapus: false }">
                            ...
                            <td class="border border-neutral-900">{{ $item['name'] }}</td>
                            <td class="border border-neutral-900">{{ $item['email'] }}</td>
                            <td class="border border-neutral-900">
                                <div x-data="{ modelOpen: false }" class="flex items-center justify-center">
                                    <form action="{{ route('data-petugas.edit', ['id' => $item['id']]) }}">
                                        <button
                                        ...
                                        <div
                                            ...
                                            <div x-cloak x-show="hapus"
                                                ...
                                                <div class="flex flex-row justify-between px-5">
                                                    <form action="{{ route('data-petugas.destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                    ...
                                                    <div
                                                        ...
                                                        <button @click="hapus = false">X</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div x-cloak x-show="detailOpen"
                                                ...
                                                <div class="flex flex-row px-3">
                                                    <div class="py-4">
                                                        ...
                                                        <div>
                                                            <p class="py-2">Nama : {{ $item['name'] }}</p>
                                                            <p class="py-2">Email : {{ $item['email'] }}</p>
                                                            <p class="py-2">Alamat : {{ $item['alamat'] }}</p>
